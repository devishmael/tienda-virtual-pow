...
function renderizarCatalogo() {
    ...
    const productosFiltrados = productos.filter(p => {
        // Control de errores: limpiamos y validamos las propiedades evitando colapsos si falta un string
        const tituloClean = (p.title || "").toLowerCase();
        const descClean = (p.description || "").toLowerCase();
        
        const cumpleTexto = tituloClean.includes(buscador) || descClean.includes(buscador);
...
function procesarCheckout(e) {
    ...
    const nroTarjeta = inputs[0].value.replace(/\s+/g, '');
    const fechaExp = inputs[1].value.trim();
    ...
    if (!regexTarjeta.test(nroTarjeta)) return alert("Error de Validación JS: La tarjeta debe tener 16 dígitos.");
    if (!regexFecha.test(fechaExp)) return alert("Error de Validación JS: Formato de fecha requerido MM/AA.");
    if (!regexCVV.test(cvv)) return alert("Error de Validación JS: El CVV debe ser de 3 dígitos.");
    ...
    lista.innerHTML = `
        <div class="flex flex-col sm:flex-row gap-3 bg-gray-100 dark:bg-gray-700 p-3 rounded-lg mb-4 text-xs">
            ...
            <div>
                ...
                <p class="text-gray-600 dark:text-gray-300 line-clamp-3">${p.description || "Sin descripción disponible."}</p>
...
function actualizarNavbar() {
    ...
    if (user) {
        ...
        if (usernameDisplay) {
            const defaultAvatar = "https://api.dicebear.com/7.x/mp/svg?seed=fallback";
            const avatarUrl = user.avatar && user.avatar.trim() !== "" ? user.avatar : defaultAvatar;
            
            usernameDisplay.innerHTML = `
                <img src="${avatarUrl}" 
                     alt="${user.nombre}" 
                     title="Perfil de ${user.nombre} (${user.rol})" 
                     class="w-8 h-8 rounded-full object-cover border-2 border-blue-500 dark:border-blue-400 shadow-xs transition transform hover:scale-110 cursor-pointer">
            `;
        }
...
function simularRecuperacion() {
    ...
    const __usuarios = JSON.parse(localStorage.getItem("usuarios")) || [];
    const encontrado = __usuarios.find(u => u.username === user);
    ...
    if (encontrado) {
        ...
        localStorage.setItem("usuarios", JSON.stringify(__usuarios));
