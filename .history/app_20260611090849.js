...
function guardarProductoAdmin(e) {
    ...
    if (id) {
        // MODO EDICIÓN: Buscamos por el ID existente
        const index = productos.findIndex(p => p.id == id);
        if (index !== -1) {
            productos[index] = { ...productos[index], title, price, stock, category, image };
        }
    } else {
        // MODO CREACIÓN: Auto-incrementar ID numérico basado en el catálogo actual
        // reduce recorre el array buscando el ID con el número más alto
        const maxId = productos.reduce((max, p) => p.id > max ? p.id : max, 0);
        const nuevoId = maxId + 1;

        const nuevo = {
            id: nuevoId, // El ID será 21, 22, 23... de forma consecutiva y limpia
            title, 
            price, 
            stock, 
            category, 
