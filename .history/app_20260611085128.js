// app.js - Núcleo de la Aplicación E-Commerce
...
function mostrarSeccion(seccionId) {
    ...
    // Protección de rutas del administrador
...
function renderizarCatalogo() {
    ...
    const precioMax = parseFloat(document.getElementById("price-filter").value) || Infinity;

    // Actualizar el label del precio dinámicamente
    document.getElementById("price-label").textContent = precioMax === Infinity ? "Cualquiera" : `$${precioMax}`;
    ...
    productosFiltrados.forEach(p => {
        ...
        div.innerHTML = `
            ...
            <div class="mt-4 space-y-2">
                <button onclick="agregarAlCarrito(${p.id})" class="w-full bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 rounded-lg text-sm transition">
                    Añadir al Carrito
                </button>
                <button onclick="verDetalleProducto(${p.id})" class="w-full border border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 py-1 rounded-lg text-xs hover:bg-gray-100 dark:hover:bg-gray-700 transition">
                    Ver Reseñas
                </button>
            </div>
        `;
...
// --- RENDERIZAR PRODUCTOS DESTACADOS EN LANDING ---
function actualizarDestacadosLanding() {
    const productos = JSON.parse(localStorage.getItem("productos")) || [];
    const contenedor = document.getElementById("productos-destacados");
    if (!contenedor) return;

    // Ordenar de mayor a menor puntuación y agarrar 3
        ...
        contenedor.innerHTML += `
            <div class="flex items-center justify-between border-b dark:border-gray-700 py-3">
                ...
                <div class="flex items-center space-x-2">
                    <button onclick="cambiarCantidadCarrito(${item.id}, -1)" class="px-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded">-</button>
                    <span class="text-sm font-semibold dark:text-white">${item.cantidad}</span>
                    <button onclick="cambiarCantidadCarrito(${item.id}, 1)" class="px-2 bg-gray-200 dark:bg-gray-700 text-gray-800 dark:text-white rounded">+</button>
                    <button onclick="eliminarDelCarrito(${item.id})" class="text-red-500 text-xs ml-2">Eliminar</button>
                </div>
            </div>
        `;
    });
...
function procesarCheckout(e) {
    ...
    // Descontar inventario local inmediatamente
    const productos = JSON.parse(localStorage.getItem("productos")) || [];
    carrito.forEach(item => {
...
function verDetalleProducto(id) {
    ...
    lista.innerHTML = p.reviews.length === 0 ? "<p class='text-gray-400 text-xs'>Sin reseñas aún.</p>" : "";
    
    p.reviews.forEach(r => {
        lista.innerHTML += `
            <div class="bg-gray-50 dark:bg-gray-700 p-2 rounded mb-2 text-xs">
                <span class="font-bold text-gray-800 dark:text-gray-200">${r.usuario}</span> (${"⭐".repeat(r.estrellas)})
                <p class="text-gray-600 dark:text-gray-400 mt-1">${r.comentario}</p>
            </div>
        `;
    });
...
// --- PANEL ADMINISTRADOR (CRUD + MÉTRICAS) ---
function renderizarAdmin() {
    ...
    document.getElementById("admin-usuarios").textContent = `${usuarios.length} Registrados`;
        ...
        tabla.innerHTML += `
            <tr class="border-b dark:border-gray-700 text-sm text-gray-900 dark:text-gray-200">
                ...
                <td class="p-2 space-x-2">
                    <button onclick="cargarFormularioEditar(${p.id})" class="text-yellow-500 hover:underline">Editar</button>
                    <button onclick="eliminarProductoAdmin(${p.id})" class="text-red-500 hover:underline">Eliminar</button>
                </td>
            </tr>
        `;
    });
    ...
    ventas.forEach((v, index) => {
        historial.innerHTML += `
            <tr class="border-b dark:border-gray-700 text-xs text-gray-900 dark:text-gray-200">
                ...
                <td class="p-2">
                    <select onchange="cambiarEstadoEnvio(${index}, this.value)" class="bg-gray-100 dark:bg-gray-700 rounded p-1">
...
// --- MANEJO DE LOGIN EN LA INTERFAZ ---
...
// --- ESCUCHADORES GLOBALES DE INICIALIZACIÓN ---
...
document.addEventListener("DOMContentLoaded", () => {
    inicializarTema();
    actualizarEstadoRed();
    guardarCarrito(); // Sincroniza contador
    actualizarNavbar();
});